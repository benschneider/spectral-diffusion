...
| `diffusion` | `num_timesteps`, `beta_schedule`, `prediction_type`, `uniform_corruption`, `uniform_corruption_scale`, `corruption_mode`, `phase_std`, `similarity_target`, `adaptive_rescale`, `fft_norm`, `snr_ratio`, `dc_scale_factor` | Standard DDPM settings plus the spectral-noise knobs consumed by `NoisePreparer`. |
