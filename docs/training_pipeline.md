...
| `AdaptiveSNRWeight` | Log-SNR adaptive weighting | fp32 EMA, α-damping, overflow mask, δ auto-tuning |
...
α-residual factor, adapts a δ·(1−α)² floor using an overflow histogram, masks
overflowed timesteps, and freezes updates if the prediction variance drifts more
than five times the input. Change-aware logging surfaces only meaningful shifts in
the balance term. Future quantised variants share the same normalisation
primitives, so BF16/INT8 runs inherit the improved guarantees.
