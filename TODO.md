...
| 🟡 | Taguchi DOE (L18/L27) | Factor registry + L18/L27 designs + GitHub Actions matrix in place | Add aggregation workflow to collate runs and publish report | Taguchi pipeline | Use `run_experiment.py --array configs/taguchi/L18_mixed.csv` or `L27_extended.csv` with the shared factor registry |
...
| ⬜ | Curriculum scheduling | `curriculum_mode` factor removed until loader/pipeline consume it | Add loader hooks for lowres warm-up (dynamic resizing) and pipeline logic for spectral-first epochs; re-enable CLI/factor once both paths emit metrics | Data loaders / TrainingPipeline | Revisit Taguchi registry + docs once the curriculum hooks are wired end-to-end |
| ⬜ | Frequency SNR study | Need end-to-end evaluation of spectral vs spatial corruption after enabling equalised noise | Run paired experiments (spatial Gaussian vs mild FFT vs strong freq-equalised) and document stability/sample-efficiency impacts | Taguchi sweep + visualization tools | Anchored to “How does explicit frequency-domain SNR control affect stability and sample efficiency?” |
