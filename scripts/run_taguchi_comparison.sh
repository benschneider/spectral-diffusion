...
OUT_DIR="${1:-$ROOT_DIR/results/taguchi_comparison}"
SUMMARY="$OUT_DIR/summary.csv"
REPORT="$OUT_DIR/taguchi_report.csv"

if [[ -d "$OUT_DIR" ]]; then
  echo "Cleaning previous comparison artifacts in $OUT_DIR"
  rm -rf "$OUT_DIR"
fi
mkdir -p "$OUT_DIR"

echo "[1/3] Running Taguchi spectral vs baseline comparison"
