#!/usr/bin/env python
"""Generate publication-ready figures from benchmark and Taguchi results."""
...
def plot_taguchi_metric_distribution(df: pd.DataFrame, metric: str, out_path: Path) -> None:
    ...
    for ax, factor in zip(axes, factor_cols):
        ...
        for patch in bp['boxes']:
            patch.set_alpha(0.7)
        ax.set(
            title=factor.replace("factor_", "").title(),
            xlabel="Level",
        )
...
def generate_figures(args) -> None:
    ...
    synthetic_df = _load_csv(args.synthetic_dir / "summary.csv") if args.synthetic_dir else None
    cifar_df = _load_csv(args.cifar_dir / "summary.csv") if args.cifar_dir else None
    taguchi_summary = _load_csv(args.taguchi_dir / "summary.csv") if args.taguchi_dir else None
    taguchi_report = _load_csv(args.taguchi_dir / "taguchi_report.csv") if args.taguchi_dir else None

    if synthetic_df is not None:
        plot_loss_metrics(
            synthetic_df,
            "Synthetic Benchmark – Loss Metrics",
            args.output_dir / "loss_metrics_synthetic.png",
        )
        plot_runtime_metrics(
            synthetic_df,
            "Synthetic Benchmark – Runtime Metrics",
            args.output_dir / "runtime_metrics_synthetic.png",
        )
    ...
    if cifar_df is not None:
        plot_loss_metrics(
            ...
            args.output_dir / "loss_metrics_cifar.png",
        )
        plot_runtime_metrics(
            cifar_df,
            "CIFAR-10 Benchmark – Runtime Metrics",
            args.output_dir / "runtime_metrics_cifar.png",
        )

    if taguchi_report is not None:
        plot_taguchi_snr(taguchi_report, args.output_dir / "taguchi_snr.png")
    if taguchi_summary is not None and taguchi_report is not None:
        plot_taguchi_metric_distribution(
            taguchi_summary,
            metric="loss_drop_per_second",
            out_path=args.output_dir / "taguchi_loss_drop_per_second.png",
        )
