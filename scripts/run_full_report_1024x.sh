...
run_synthetic() {
  echo "[1/5] Synthetic benchmarks (1024x1024 images)"
...
run_feature_ablation() {
  ...
  describe_run "$ROOT_DIR/configs/benchmark_spectral_cifar.yaml" "cifar_1024x1024_spectral_plain" "spectral"
  python "$ROOT_DIR/train.py" \
    --config "$ROOT_DIR/configs/benchmark_spectral_cifar.yaml" \
    --output-dir "$ABL_DIR" \
    --run-id "cifar_1024x1024_spectral_plain" \
    --variant spectral

  tmp_cfg_ablation="$(mktemp "$ABL_DIR/cifar_1024_ablation_XXXX.yaml")"
  augment_config "$ROOT_DIR/configs/benchmark_spectral_cifar.yaml" "$tmp_cfg_ablation"
  describe_run "$tmp_cfg_ablation" "cifar_1024x1024_spectral_uniform" "spectral+uniform"
  python "$ROOT_DIR/train.py" \
    --config "$tmp_cfg_ablation" \
    --output-dir "$ABL_DIR" \
    --run-id "cifar_1024x1024_spectral_uniform" \
    --variant spectral
}
